...
/*! @brief Callback to toggle the green LED when the PIT interrupt occurs
 *
 */
void Pit0Callback() {
...
static void InitModulesThread(void* pData)
{
  ...
  */
  ...
  // Initialise the low power timer to tick 5 seconds
  LPTMRInit(500);

  // Initialise the LEDs for testing
  LEDs_Init();

  uint32_t halfSecondToNanoSeconds = 500000000;
  uint32_t twoSecondsToNanoseconds = 2000000000;

  // Initialize and Set the PIT module
  if (PIT_Init(CPU_BUS_CLK_HZ, Pit0Callback, NULL, 0)) {
    PIT_Set(halfSecondToNanoSeconds, true, 0);
  }

  if (PIT_Init(CPU_BUS_CLK_HZ, Pit1Callback, NULL, 1)) {
    PIT_Set(twoSecondsToNanoseconds, true, 1);
  }
